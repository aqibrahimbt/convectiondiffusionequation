...
class EnrichmentProxy(CoefficientFunction):
    ...
    def x(self):
        return sum([self.func[i]* self.enr_list[i-1] for i in range(1, len(self.enr_list)+1) or []])
    ...
    def grad(self):
        ...
        for i in range(1, len(self.enr_list)+1) or []:
...
class EnrichmentProxy_VOL(CoefficientFunction):
    ...
    def x(self):
        return sum([self.func[2+2*i]*self.enr_list[i] for i in range(0, len(self.enr_list)) or []])
    ...
    def grad(self):
        mygrad = ngsolvegrad(self.func[0])
        for i in range(len(self.enr_list)) or []:
...
class EnrichmentProxy_FAC(CoefficientFunction):
    ...
    def __call__(self):
        return self.func[1] + sum([self.func[3 + 2*i]*self.enr_list[i] for i in range(0, len(self.enr_list)) or []])

    def x(self):
        return sum([self.func[3 + 2*i]*self.enr_list[i] for i in range(0, len(self.enr_list)) or []])
...
'''Mark dofs in the mesh to be enriched'''
...
def is_pos_def(A):
    if np.array_equal(A, A.T):
        try:
            ...
            print('cond= ', np.linalg.cholesky(A))
            return True
